...
object OpenRouterEngine {
    ...
    // Default OpenRouter API configuration (100% Free models tested & working)
    ...
    const val FALLBACK_MODEL = "openai/gpt-oss-20b:free"
    ...
    fun processVoicePrompt(
        ...
        val systemPrompt = """
            You are a multilingual AI voice reminder engine with deep understanding of Sinhala (සිංහල), English, Tamil, and Singlish/Tanglish.
            ...
            - "විනාඩි" / "winadi" = minutes (e.g. "තව විනාඩි 10කින්" / "thawa winadi 10kin" = in 10 minutes)
            - "පැය" / "paya" = hours (e.g. "පැය 2කින්" = in 2 hours)
            - "බෙහෙත් බොන්න" = Take medicine
            - "කතා කරන්න" / "call ekak denna" = Call

            Rules:
            1. Parse the user's intent into a concise, clean task title in English (clean_idea).
            2. If the user mentions relative or absolute time in Sinhala or English, calculate the exact future target timestamp in 'yyyy-MM-dd HH:mm' format.
            3. If no time is specified, return empty string "" for target_time_formatted.
            
            You MUST return RAW JSON ONLY:
            {
              "clean_idea": "Synthesized task in English",
